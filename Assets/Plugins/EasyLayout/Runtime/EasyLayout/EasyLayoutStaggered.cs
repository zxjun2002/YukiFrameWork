namespace EasyLayoutNS
{
	...
	public class EasyLayoutStaggered : EasyLayoutFlexOrStaggered
	{
		...
		{
			var block_count = IsHorizontal
				? ElementsGroup.ItemsInRow(block_index)
				: ElementsGroup.ItemsInColumn(block_index);
			if (IsHorizontal)
			{
				ElementsGroup.SetPosition(element, block_index, block_count);
			}
			else
			{
				ElementsGroup.SetPosition(element, block_count, block_index);
			}
