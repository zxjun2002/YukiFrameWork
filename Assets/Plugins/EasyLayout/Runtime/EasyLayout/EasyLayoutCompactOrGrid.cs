...
#if UNITY_EDITOR
			if (MaxColumnsWidths.IsValid(out var c_index))
			{
				throw new ArgumentException("MaxColumnsWidths value has NaN value at position: " + c_index);
			}

			if (RowsWidths.IsValid(out var r_index))
			{
				throw new ArgumentException("RowsWidths value has NaN value at position: " + r_index);
			}
#endif
...
#if UNITY_EDITOR
				if (row_cell_max_size.IsValid())
				{
					throw new ArgumentException("row_cell_max_size value has NaN component: " + row_cell_max_size);
				}
#endif
...
#if UNITY_EDITOR
			if (MaxRowsHeights.IsValid(out var c_index))
			{
				throw new ArgumentException("MaxRowsHeights value has NaN value at position: " + c_index);
			}

			if (ColumnsHeights.IsValid(out var r_index))
			{
				throw new ArgumentException("ColumnsHeights value has NaN value at position: " + r_index);
			}
#endif
...
#if UNITY_EDITOR
				if (column_cell_max_size.IsValid())
				{
					throw new ArgumentException("column_cell_max_size value has NaN component: " + column_cell_max_size);
				}
#endif
